...
Connections.lifeMove = RunService.Heartbeat:Connect(function()
    ...
    if dist > 1 then
        ...
        local speed =
            (lifeState == 0 and Values.SpeedToL1) or
        ...
        hrp.AssemblyLinearVelocity =
            Vector3.new(direction.X,0,direction.Z).Unit * speed
...
Connections.rightMove = RunService.Heartbeat:Connect(function()
    ...
    if dist > 1 then
        ...
        local speed =
            (rightState == 0 and Values.SpeedToR1) or
...
local function stopAutoMovement()
    ...
    if Connections.lifeMove then
        Connections.lifeMove:Disconnect()
        Connections.lifeMove = nil
    end

    if Connections.rightMove then
        Connections.rightMove:Disconnect()
        Connections.rightMove = nil
    end

    autoStealActiveForPaths = false
    waitingForSteal = false
    lifeState = 0
    rightState = 0
    waitAtTarget = false
    ...
    for _,p in pairs(Players:GetPlayers()) do
        if p ~= lp then
            ...
            if c then
                ...
                local enemy = c:FindFirstChild("HumanoidRootPart")
                local hum = c:FindFirstChildOfClass("Humanoid")

                if enemy and hum and hum.Health > 0 then

                    local d = (enemy.Position - root.Position).Magnitude
